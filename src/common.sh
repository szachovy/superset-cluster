...
initialize_nodes() {
  ...
  mysql_superset_password=$(ssh superset@${primary_node} "tr --delete --complement 'a-z' </dev/urandom | head -c 12")
  ...
  for mgmt_node in "${mgmt_nodes[@]}"; do
    ...
    ssh superset@${mgmt_node} "echo ${mysql_superset_password} | docker secret create mysql_superset_password -"

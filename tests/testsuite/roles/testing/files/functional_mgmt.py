...
class MgmtNodeFunctionalTests(utils.ContainerUtilities, metaclass=utils.Overlay):
    def __init__(self) -> None:
        self.arguments: argparse.Namespace = ArgumentParser().parse_arguments()
        super().__init__(node=self.arguments.mgmt_hostname)
        self.mgmt_primary_node: str = f"{self.arguments.node_prefix}-0"
        self.mgmt_secondary_node: str = f"{self.arguments.node_prefix}-10"
        self.mysql_primary_node: str = f"{self.arguments.node_prefix}-1"
        self.mysql_secondary_nodes: list = [f"{self.arguments.node_prefix}-2", f"{self.arguments.node_prefix}-3"]
        self.mysql_node_disaster_delay: int = 300
        self.mgmt_node_disaster_delay: int = 12

    @utils.Overlay.post_init_hook
    ...
    def status_swarm(self):
        ...
        assert swarm_info['ControlAvailable'] is False, f'The {self.arguments.node_prefix}-0 is not supposed to be a Swarm manager, but it is'

...
class MgmtNodeFunctionalTests(container.ContainerConnection, metaclass=decorators.Overlay):
    ...
    def status_cluster(self) -> None | AssertionError:
        command: str = f"""
            mysqlsh
    ...
    @decorators.Overlay.run_selected_methods_once
    def status_routers(self) -> None | AssertionError:
        command: str = f"""
            mysqlsh
                --login-path={self.mysql_secondary_nodes[0]}
                --interactive
        ...
        assert \
            ...
            f"""MySQL Mgmt routers are offline or not attached,
                ...
                \nCommand: {command}\nReturned: {routers_status_output}
            """

    def check_after_disaster(self) -> None | AssertionError:

...
class Redis(container.ContainerConnection, metaclass=decorators.Overlay):
    ...
    def status(self) -> None | AssertionError:
        command: str = "python3 -c 'import redis; print(redis.StrictRedis(host=\"redis\", port=6379).ping())'"
    ...
    def fetch_query_result(self, results_key: float) -> bool | AssertionError:
        command: str = f"python3 -c 'import redis; print(redis.StrictRedis(host=\"redis\", port=6379).get(\"{results_key}\"))'"
...
class Celery(container.ContainerConnection, metaclass=decorators.Overlay):
    ...
    def status(self) -> None | AssertionError:
        ...
        test_connection: bytes = self.run_command_on_the_container(command)
        assert self.find_in_the_output(test_connection, b"{'ok': 'pong'}"), f'The Celery process in the {self.superset_container} container on is not responding\nCommand: {command}\nReturned: {test_connection}'

    @decorators.Overlay.run_selected_methods_once
    def status_cache(self) -> None | AssertionError:
        command: str = f"python3 -c 'import celery; print(celery.Celery(\"tasks\", broker=\"{self.celery_broker}\").control.inspect().conf())'"
    ...
    def find_processed_queries(self) -> bool | AssertionError:
        command = f"python3 -c 'import celery; print(celery.Celery(\"tasks\", broker=\"{self.celery_broker}\").control.inspect().stats())'"
...
class Superset(container.ContainerConnection, metaclass=decorators.Overlay):
    ...
    def login_to_superset_api(self) -> str | AssertionError:
        ...
        command: str = f"curl --cacert /app/server_certificate.pem --silent --url {self.api_default_url}/security/login --header '{headers}' --data '{payload}'"
    ...
    def login_to_superset(self) -> dict[str, str] | AssertionError:
        command: str = f"curl --cacert /app/server_certificate.pem --include --url {self.api_default_url}/security/csrf_token/ --header '{self.api_authorization_header}'"
        ...
        command: str = f"curl --location --cacert /app/server_certificate.pem --include --url https://{self.virtual_ip_address}/login/ --header 'Cookie: session={session_request_cookie}' --data 'csrf_token={csrf_token}'"
    ...
    def run_query(self) -> float | AssertionError:
        ...
        command: str = f"curl --location --cacert /app/server_certificate.pem --silent {self.api_default_url}/sqllab/execute/ --header 'Content-Type: application/json' --header '{self.api_authorization_header}' --header '{self.api_session_header}' --header '{self.api_csrf_header}' --header 'Referer: https://{self.virtual_ip_address}' --data '{payload}'"
        sqllab_run_query: bytes = self.run_command_on_the_container(command)
        assert not self.find_in_the_output(sqllab_run_query, b'"msg"'), f'SQL query execution failed\nCommand: {command}\nReturned: {sqllab_run_query}'
        assert not self.find_in_the_output(sqllab_run_query, b'"message"'), f'Could not execute SQL query\nCommand: {command}\nReturned: {sqllab_run_query}'
    ...
    def get_query_results(self, dttm_time_query_identifier: float):
        ...
        assert query_result.get("result")[0]['state'] == 'success', f'Could not find query state or returned unsuccessful\nCommand: {command}\nReturned decoded: {query_result}'
        results_key: str = f"superset_results{query_result.get('result')[0]['resultsKey']}"
        assert self.redis.fetch_query_result(results_key), f'Query result with the {results_key} key can not be found in Redis after\nCommand: {command}'
        assert self.celery.find_processed_queries(), f'Query seems to be processed outside Celery worker after\nCommand: {command}'

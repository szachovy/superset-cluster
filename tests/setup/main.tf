...
resource "null_resource" "manage_ssh" {
  provisioner "local-exec" {
    when    = create
    command = <<-EOT
      [ -f id_rsa ] || ssh-keygen -t rsa -b 2048 -f id_rsa -N ''
      chmod 600 id_rsa.pub
      [ -f $HOME/.ssh/config ] || mkdir --parents $HOME/.ssh && touch $HOME/.ssh/config
      grep -q "### Terraform autogenerated content start ###" $HOME/.ssh/config || \
...
resource "docker_container" "nodes" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      grep -q "^Host $HOSTNAME$" $HOME/.ssh/config 2>/dev/null || \
      echo "Host $HOSTNAME
        Hostname $IP_ADDRESS
...
resource "null_resource" "finish_configuration" {
  provisioner "local-exec" {
    command = <<-EOT
      grep -q "### Terraform autogenerated content end ###" $HOME/.ssh/config || \

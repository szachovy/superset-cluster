...
resource "docker_container" "nodes" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      docker exec $HOSTNAME /bin/bash -c "wget --quiet https://bootstrap.pypa.io/get-pip.py && python3 get-pip.py && pip install --quiet --no-cache-dir --user --requirement /opt/superset-cluster/src/requirements.txt"
      echo "Host $HOSTNAME
        Hostname $IP_ADDRESS
